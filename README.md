...
- `prompt`（可选）
- `urls`（可选，来自多图上传，发送为 data URL 数组）
- `aspectRatio`: `auto` / `1:1` / `16:9` / `9:16` / `4:3` / `3:4` / `3:2` / `2:3` / `5:4` / `4:5` / `21:9`
- `imageSize`: `1K` / `2K` / `4K`

## 说明
- 可使用“清空参考图”按钮一次清空已上传内容。
- 接口响应控件会同时展示请求参数与接口返回，方便对照调试。
- 支持拖拽/点击上传参考图；每次新增会自动追加到已上传列表（可分批连续上传），并显示固定宽度缩略图预览。
- 参考图组件支持多图上传。
- 提交时会校验：`prompt` 与 `urls` 不能同时为空。
- 右侧会优先展示接口返回中的图片（支持 URL 或 Base64，也支持 SSE `data:` 事件流里最终结果）。
- 接口响应面板仅用于调试，已缩小高度并把空间更多让给输出图片。
